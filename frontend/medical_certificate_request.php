...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $from_date = DateTime::createFromFormat('d-m-Y', $_POST['from_date'])->format('Y-m-d');
    $to_date = DateTime::createFromFormat('d-m-Y', $_POST['to_date'])->format('Y-m-d');
    ...
    if ($status == "pending") {
        ...
        $query = "INSERT INTO medical_certificate 
        ( patient_id, doctor_id, description, request_date_time, status, payment, from_date, to_date, certificate_type, reason, illness_description, document_path, name, email, phone, dob, address, card_number, security_code, expiration_date, country, gender, title) 
        VALUES 
        (?, ?, ?, NOW(), 'assigned', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssisssssssssssssssss", $Id, $doctorId, $description, $payment, $from_date, $to_date, $certificate_type, $reason, $illness_description, $filePath, $name, $email, $phone, $dob, $address, $card_number, $security_code, $expiration_date, $country, $gender, $title);
    }
...
<div class="container-fluid m-0 p-0">
    <div class="content">
        <div class="request-form">
            <form
                action="medical_certificate_request.php<?php echo isset($appointment['status']) ? '?status=' . $appointment['status'] . '&doctor_id=' . $appointment['doctor_id'] : '' ?>"
                id="requestForm" method="POST" enctype="multipart/form-data">
                <h2 class="text-center mt-3">Request Medical Certificate</h2>
                <div class="form-step active " id="step1">

                    <div class="row">
                        <label><b> Which type of Medical Certificate are you
                                after?</b> <b class="required-asterisk">*</b></label>
                ...
                <div class="form-step" id="step2">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-6">
                            <div class="input-group">
                                <i class="fa-solid fa-cake-candles" style="z-index: 6;"></i> <!-- Icon for birthday -->
                                <input type="text" style="cursor: pointer;" name="dob" id="dob" class="width-hundred"
