...
// Get Referral ID
$referral_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Get existing data for the referral
    $query = "SELECT notes FROM referrals WHERE id = ?";
    ...
    $query = "UPDATE referrals SET notes = ? WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("si", $json_data, $referral_id);
    $stmt->execute();
    $stmt->close();
...
<div class="second-section">
    <section>
        ...
        <div class="container mt-5">
            <h2 class="page-header">Edit Notes for Referral ID <?php echo htmlspecialchars($referral_id); ?></h2>
