...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['message'])) {
    ...
    $sender = 'Doctor'; // Since the patient is sending the message
    ...
    $stmt->execute();
    $stmt->close();
    require '../emails/chat_notification_mail_for_patient.php';
    // Redirect to prevent resubmission on page refresh
    header("Location: chat.php?certificate_id={$certificate_id}&patient_name={$patient_name}&email={$email}");
    exit();
}
