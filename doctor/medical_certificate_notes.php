...
// Get medical_certificate ID
$medical_certificate_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Get existing data for the medical_certificate
    $query = "SELECT notes FROM medical_certificate WHERE id = ?";
    ...
    $existing_data['Clinical Findings'] = $clinical_findings;
    ...
    $query = "UPDATE medical_certificate SET notes = ? WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("si", $json_data, $medical_certificate_id);
    $stmt->execute();
    $stmt->close();
...
$clinical_findings = isset($existing_data['Clinical Findings']) ? htmlspecialchars($existing_data['Clinical Findings']) : '';
...
<div class="second-section">
    <section>
        ...
        <div class="container mt-5">
            <h2 class="page-header">Edit Notes for Medical Certificate ID
                <?php echo htmlspecialchars($medical_certificate_id); ?>
            </h2>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between">
                    ...
                    <div class="form-group mt-5" style="width: 100%;">
                        <label for="clinical_findings"><strong>Clicnical Findings:</strong></label>
                        <textarea name="clinical_findings" id="clinical_findings" class="form-control"
                            rows="3"><?php echo htmlspecialchars($clinical_findings); ?></textarea>
                    </div>
                </div>
